-- Reservation schema patch (idempotent)
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE conname = 'ProductReservation_productId_fkey'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'ProductReservation_buyerId_fkey'
    ) THEN
        ALTER TABLE public."ProductReservation"
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'ProductReservation_sellerId_fkey'
    ) THEN
        ALTER TABLE public."ProductReservation"
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'ReservationUserPenalty_userId_fkey'
    ) THEN
