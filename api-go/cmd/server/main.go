...
func main() {
	...
	productExpirySvc := service.NewProductExpiryWorker(cfg, prodRepo, supSvc)
	productExpirySvc.StartExpiryWorker(ctx)
	promoRepo := repository.NewPromotionPG(pool)
	promoSvc := service.NewPromotionService(promoRepo, supSvc)
